<nav>
    <div class="inquiry_line">
        <a href="#">공동구매는 soundwave@sound-wave.co.kr 로 문의주세요!</a>
    </div>
    <div class="nav_clone_content">
        <div class="select_down">
            <div class="btn">
                <button type="button">KOR</button>
                <img src="../images/bottom_arrow.png" alt="">
            </div>
            <ul class="select_lang">
                <li><a href="#">KOR</a></li>
                <li><a href="#">ENG</a></li>
                <li><a href="#">JPN</a></li>
                <li><a href="#">CHN</a></li>
            </ul>
            <div class="nav_close"><a href="#"><img src="../images/close.png" alt="전체메뉴 닫기"></a></div>
        </div>
        <ul class="nav_clone_list">
            <li><a href="#">회원가입</a></li>
            <li><a href="#">주문조회</a></li>
            <li><a href="./login.html">로그인</a></li>
        </ul>
    </div>
    <div class="nav_mid">
        <a href="#" class="menu_btn"><span></span><span></span><span></span><span></span></a>
        <h1><a href="../index.html"><img src="../images/re_logo.png" alt="SOUNDWAVE"></img></a></h1>
        <ul class="nav_mid_right">
            <li>
                <a href="#" class="search_a"><img src="../images/search_ico.png" alt="검색"></a>
                <form id="searchBarForm" name="" method="get">
                    <h2>SEARCH</h2>
                    <div class="search_bar_line">
                        <input type="text" id="search_bar" name="search_bar">
                        <button type="submit"></button>
                    </div>
                    <p>인기검색어</p>
                </form>
            </li>
            <li><a href="#"><img src="../images/cart_ico.png" alt="장바구니"></a></li>
            <!-- <li><a href="#"><img src="../images/user_ico.png" alt="내 정보"></a></li> -->
        </ul>
    </div>
    <div class="nav_btm">
        <ul class="gnb">
            <li>
                <a href="./new.html">NEW<span></span></a>
                
                <ul class="sub">
                    <li><a href="./new.html">PRE-ORDER</a></li>
                </ul>
            </li>
            <li>
                <a href="./new.html">MUSIC<span></span></a>
                <ul class="sub">
                    <li><a href="./new.html">CD & DVD</a></li>
                    <li><a href="./new.html">PLVE</a></li>
                    <li><a href="./new.html">ETC.</a></li>
                </ul>
            </li>
            <li>
                <a href="./new.html">MD<span></span></a>
                <ul class="sub">
                    <li><a href="./new.html">ARTIST</a></li>
                    <li><a href="./new.html">LIGHTSTICK</a></li>
                    <li><a href="./new.html">SEASON’S GREETING</a></li>   
                </ul>
            </li>
            <li>
                <a href="./new.html">EVENT<span></span></a>
                <ul class="sub">
                    <li><a href="./new.html">SPECIAL GIFT</a></li>
                    <li><a href="./new.html">FAN EVENT</a></li>
                    <li><a href="./new.html">LUCKY DRAW</a></li>
                    <li><a href="./new.html">ETC</a></li>
                </ul>
            </li>
            <li>
                <a href="./new.html">NOTICE<span></span></a>
                <ul class="sub">
                    <li><a href="./new.html">EVENT NOTICE</a></li>
                    <li><a href="./new.html">당첨자발표</a></li>
                </ul>
            </li>
            <li>
                <a href="./new.html">SOUNDWAVE TV<span style="display: none;"></span></a>
                <ul class="sub non_sub"></ul>
            </li>
            <li>
                <a href="./login.html">CS CENTER<span></span></a>
                <ul class="sub">
                    <li><a href="./login.html">1:1 문의</a></li>
                    <li><a href="./login.html">FAQ</a></li>
                </ul>
            </li>
        </ul>
    </div>
    <div class="deco_underline"></div>
</nav>
<div class="nav_clone_area"></div>